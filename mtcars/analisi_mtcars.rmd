...
for (i in to_describe){
  ...
  for (name in names(summary)) {
    printn(name, ": ", mode="1", sameLn = T)
    printn(summary[[name]])
  }
  printn('Deviazione Standard', mode="1", sameLn = T)
  printn(sd(curcol))
  printn("Range:" , mode="1", sameLn = T)
  printn(diff(range(curcol)))
  printn('------------------------------------------')
}
```
